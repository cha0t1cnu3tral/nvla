...
- Native `NVDA+1`, `NVDA+C`, `NVDA+F2`, `NVDA+F12`, `NVDA+T`, `NVDA+Tab`,
  `NVDA+B`, `NVDA+H`, and `NVDA+Q` preview commands for input help, clipboard
  reporting, passing the next key sequence through, time and date, the active
  window title, focused object, a bounded active-tree read-through, command
  help, and clean preview shutdown.
